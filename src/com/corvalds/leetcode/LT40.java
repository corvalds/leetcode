 ...
 * 问题思路：采取回溯算法思路，每次目标值都减去数组中的某个元素，直至目标值为负或者为0，当目标值为负时，意味着之前遍历的数组元素的组合累加和不等于目标值；当目标值为0时，意味着之前遍历的数组元素的组合累加和等于目标值，可以作为结果，但此时有一个去重问题需要处理，基本的思路先对数组进行排序，每次目标值减去数组元素后，下次减去的只能是当前错做的数组元素后面的元素（相当于缩小选择集），同时这里需要注意如果后一个元素和当前元素相同，则不需要操作，否则会导致重复组合<br/>
